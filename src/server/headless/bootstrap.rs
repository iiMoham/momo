...
pub fn run_server() -> io::Result<()> {
    ...
    let _api_server = match api::start_server_with_stop_control(
        ...
        Err(err) if err.kind() == io::ErrorKind::AddrInUse => {
            eprintln!("error: momo server is already running");
...
fn print_ready_message(api_socket: &Path, client_socket: &Path) {
    eprintln!("momo server running; you can use any momo CLI command in another terminal.");
    ...
    eprintln!("did you mean to open the MoMo TUI? run `momo`; you do not need `momo server`.");
}

/// Initialize logging for the server process.
fn init_logging() {
    crate::logging::init_file_logging("momo-server.log");
}

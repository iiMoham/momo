...
pub(super) fn mismatch_response(
    ...
    let message = if client_protocol > server_protocol {
        format!(
            "client protocol {client_protocol} is newer than server protocol {server_protocol}; restart the MoMo server before using this command. {restart_guidance}"
        )
    } else {
        format!(
            "client protocol {client_protocol} is older than server protocol {server_protocol}; upgrade the MoMo client before using this command"
        )
    };
...
mod tests {
    ...
    fn newer_server_error_tells_user_to_upgrade_client() {
        ...
        assert!(response.error.message.contains("upgrade the MoMo client"));

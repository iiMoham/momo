...
pub(super) fn command() -> Command {
    let command = Command::new(crate::brand::CLI_NAME)
        ...
        .arg(option("remote", "TARGET").help("Attach through SSH to a remote MoMo server"))
        .arg(
...
fn write_requested_help(
    ...
    let mut path = vec![crate::brand::CLI_NAME.to_string()];
    ...
    Command::new("notification")
        ...
        .subcommand(
            Command::new("open-target")
                ...
                .after_help(
                    "MoMo puts this command on clickable system notifications (macOS with terminal-notifier); you rarely run it yourself.",
                ),
        )
}
    ...
    Command::new("agent")
        ...
        .subcommand(
            Command::new("read")
                ...
                .override_usage("momo agent read <TARGET> [OPTIONS]")
        ...
        .subcommand(
            Command::new("prompt")
                ...
                .override_usage("momo agent prompt <TARGET> <TEXT> [OPTIONS]")
                .arg(required("target", "TARGET"))
        ...
        .subcommand(
            Command::new("rename")
                ...
                .override_usage("momo agent rename <TARGET> <NAME>|--clear")
                .arg(required("target", "TARGET"))
        ...
        .subcommand(
            Command::new("wait")
                ...
                .override_usage("momo agent wait <TARGET> [OPTIONS]")
                .arg(required("target", "TARGET"))
                .arg(
                    option("until", "STATUS")
                        .action(ArgAction::Append)
        ...
        .subcommand(
            Command::new("attach")
                ...
                .override_usage("momo agent attach <TARGET> [OPTIONS]")
                .arg(required("target", "TARGET"))
                .arg(flag("takeover")),
        )
        .subcommand(
            Command::new("start")
                ...
                .override_usage(
                    "momo agent start <NAME> --kind <KIND> --pane <ID> [OPTIONS] [-- [AGENT_ARG]...]",
                )
    ...
    Command::new("pane")
            ...
            Command::new("input")
                ...
                .arg(
                    option("right-click", "TARGET")
                        .value_parser(right_click_targets())
                        .required(true),
                ),
        )
        .subcommand(
            Command::new("split")
                ...
                .arg(path_option("cwd", "PATH"))
                .arg(env_option())
                .arg(option("right-click", "TARGET").value_parser(right_click_targets()))
                .arg(flag("focus"))
                .arg(flag("no-focus")),
        )
        .subcommand(
        ...
        .subcommand(
            Command::new("send-text")
                ...
                .arg(required("text", "TEXT"))
                .after_help(
                    "next: momo pane run <PANE_ID> <COMMAND> sends text and Enter in one call",
                ),
        )
        .subcommand(
            Command::new("send-keys")
...
/// `momo` opens MoMo's own menu on right-click; `herdr` is the older spelling
/// and stays accepted (hidden) so existing scripts keep working.
fn right_click_targets() -> clap::builder::PossibleValuesParser {
    clap::builder::PossibleValuesParser::new([
        clap::builder::PossibleValue::new("momo"),
        clap::builder::PossibleValue::new("pane"),
        clap::builder::PossibleValue::new("herdr").hide(true),
    ])
}
...
mod tests {
        ...
        for path in paths {
            for flag in ["-h", "--help"] {
                ...
                assert!(
                    ...
                    "help was not handled for momo {} {flag}",
                    path.join(" ")
                );
                let output = String::from_utf8(output).unwrap();
                assert!(
                    output.contains(&format!("Usage: momo {}", path.join(" "))),
                    "unexpected help for momo {}: {output}",
                    path.join(" ")
                );
            }
        }
        ...
        for (path, options) in [
            ...
            for option in options {
                assert!(
                    ...
                    "momo {} --{option} should be required",
                    path.join(" ")
                );
            }
        }
    ...
    fn agent_rename_requires_exactly_one_name_or_clear() {
        ...
        assert!(String::from_utf8(help)
            .unwrap()
            .contains("Usage: momo agent rename <TARGET> <NAME>|--clear"));
    }
    ...
    fn worktree_json_compatibility_flag_stays_out_of_public_spec() {
        ...
        for subcommand in ["list", "create", "open", "remove"] {
            ...
            assert!(
                ...
                "momo worktree {subcommand} should not advertise --json"
            );
        }
    }
    ...
    fn agent_resources_appear_on_command_groups_but_not_leaf_commands() {
        for group in ["agent", "pane", "workspace", "terminal"] {
            ...
            assert!(
                ...
                "momo {group} is missing agent resources: {help}"
            );
        }
    ...
    fn next_step_hints_render_without_replacing_existing_after_help() {
        ...
        assert!(
            agent_start.contains("next: momo agent prompt <TARGET> <TEXT> --wait"),

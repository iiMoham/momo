const API_SCHEMA_JSON: &str = include_str!("../../docs/api/momo-api.schema.json");
...
fn api_snapshot(args: &[String]) -> std::io::Result<i32> {
    if !args.is_empty() {
        eprintln!("usage: momo api snapshot");
        return Ok(2);
    }
...
fn schema_summary_text() -> std::io::Result<String> {
    ...
    Ok(format!(
        "MoMo API schema\nprotocol: {}\nschema_version: {}\nschemas: {}\n\nUse `momo api schema --json` to print the full schema.\nUse `momo api schema --output PATH` to write it to a file.\n",
        protocol,
        schema_version,
        schemas.join(", ")
    ))
...
fn print_api_help() {
    eprintln!("momo api commands:");
    eprintln!("  momo api snapshot");
    eprintln!("  momo api schema [--json | --output PATH]");
}

fn print_api_schema_help() {
    eprintln!("usage: momo api schema [--json | --output PATH]");
}
...
mod tests {
    ...
    fn schema_summary_text_stays_human_sized() {
        ...
        assert!(text.contains("MoMo API schema"));
        assert!(text.contains("Use `momo api schema --json`"));
        assert!(text.len() < 400);
    }
}

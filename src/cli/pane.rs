...
fn pane_get(args: &[String]) -> std::io::Result<i32> {
    let Some(raw_pane_id) = args.first() else {
        eprintln!("usage: momo pane get <pane_id>");
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("usage: momo pane get <pane_id>");
...
fn parse_pane_neighbor_args(args: &[String]) -> Result<PaneNeighborParams, String> {
    ...
    let Some(direction) = direction else {
        return Err(
            "usage: momo pane neighbor --direction left|right|up|down [--pane ID|--current]".into(),
        );
    };
...
fn parse_pane_focus_args(args: &[String]) -> Result<PaneFocusDirectionParams, String> {
    let params = parse_pane_neighbor_args(args).map_err(|_| {
        "usage: momo pane focus --direction left|right|up|down [--pane ID|--current]".to_string()
    })?;
...
fn parse_pane_resize_args(args: &[String]) -> Result<PaneResizeParams, String> {
    ...
    let Some(direction) = direction else {
        return Err(
            "usage: momo pane resize --direction left|right|up|down [--amount FLOAT] [--pane ID|--current]"
                .into(),
        );
    };
...
fn pane_rename(args: &[String]) -> std::io::Result<i32> {
    let Some(raw_pane_id) = args.first() else {
        eprintln!("usage: momo pane rename <pane_id> <label>|--clear");
        return Ok(2);
    };
    if args.len() < 2 {
        eprintln!("usage: momo pane rename <pane_id> <label>|--clear");
...
fn parse_pane_read_args(args: &[String]) -> Result<PaneReadParams, String> {
    const USAGE: &str = "usage: momo pane read <pane_id> [--source visible|recent|recent-unwrapped|detection] [--lines N] [--format text|ansi] [--ansi] [--raw]";
...
fn parse_pane_input_args(
    ...
    const USAGE: &str =
        "usage: momo pane input [<pane_id>|--pane ID|--current] --right-click momo|pane";
...
fn parse_right_click_target(value: &str) -> Result<PaneRightClickTarget, String> {
    match value {
        "momo" | "herdr" => Ok(PaneRightClickTarget::Herdr),
...
fn parse_pane_split_args(
    ...
    let Some(direction) = direction else {
        return Err(
            "usage: momo pane split [<pane_id>|--pane ID|--current] --direction right|down [--ratio FLOAT] [--cwd PATH] [--env KEY=VALUE] [--right-click momo|pane] [--focus] [--no-focus]"
                .into(),
        );
    };
...
fn pane_move_usage() -> String {
    "usage: momo pane move <pane_id> --tab <tab_id> --split right|down [--target-pane ID] [--ratio FLOAT] [--focus|--no-focus]\n       momo pane move <pane_id> --new-tab [--workspace ID] [--label TEXT] [--focus|--no-focus]\n       momo pane move <pane_id> --new-workspace [--label TEXT] [--tab-label TEXT] [--focus|--no-focus]"
        .into()
}
...
fn parse_pane_swap_args(args: &[String]) -> Result<PaneSwapParams, String> {
    ...
    match (directional, explicit) {
        ...
        _ => Err(
            "usage: momo pane swap --direction left|right|up|down [--pane ID|--current]\n       momo pane swap --source-pane ID --target-pane ID"
                .into(),
        ),
    }
}
...
fn pane_close(args: &[String]) -> std::io::Result<i32> {
    let Some(raw_pane_id) = args.first() else {
        eprintln!("usage: momo pane close <pane_id>");
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("usage: momo pane close <pane_id>");
...
fn pane_send_text(args: &[String]) -> std::io::Result<i32> {
    if args.len() < 2 {
        eprintln!("usage: momo pane send-text <pane_id> <text>");
        return Ok(2);
    }
...
fn pane_send_keys(args: &[String]) -> std::io::Result<i32> {
    if args.len() < 2 {
        eprintln!("usage: momo pane send-keys <pane_id> <key> [key ...]");
        return Ok(2);
    }
...
fn pane_run(args: &[String]) -> std::io::Result<i32> {
    if args.len() < 2 {
        eprintln!("usage: momo pane run <pane_id> <command>");
        return Ok(2);
    }
...
fn parse_pane_wait_output_args(args: &[String]) -> Result<PaneWaitForOutputParams, String> {
    const USAGE: &str = "usage: momo pane wait-output <pane_id> (--match TEXT | --regex PATTERN) [--source visible|recent|recent-unwrapped] [--lines N] [--timeout MS] [--raw]";
...
fn pane_report_agent(args: &[String]) -> std::io::Result<i32> {
    const USAGE: &str = "usage: momo pane report-agent <pane_id> --source ID --agent LABEL --state idle|working|blocked|unknown [--message TEXT] [--seq N] [--agent-session-id ID] [--agent-session-path PATH]";
...
fn pane_report_agent_session(args: &[String]) -> std::io::Result<i32> {
    const USAGE: &str = "usage: momo pane report-agent-session <pane_id> --source ID --agent LABEL [--seq N] [--agent-session-id ID] [--agent-session-path PATH] [--session-start-source SOURCE]";
...
fn pane_release_agent(args: &[String]) -> std::io::Result<i32> {
    let Some(raw_pane_id) = args.first() else {
        eprintln!("usage: momo pane release-agent <pane_id> --source ID --agent LABEL [--seq N]");
        return Ok(2);
    };
...
fn pane_report_metadata(args: &[String]) -> std::io::Result<i32> {
    let Some(raw_pane_id) = args.first() else {
        eprintln!("usage: momo pane report-metadata <pane_id> --source ID [--agent LABEL] [--applies-to-source ID] [--title TEXT|--clear-title] [--display-agent TEXT|--clear-display-agent] [--state-label STATUS=TEXT] [--clear-state-labels] [--token NAME=VALUE] [--clear-token NAME] [--seq N] [--ttl-ms N]");
        return Ok(2);
    };
...
const PANE_WATCH_USAGE: &str = "usage: momo pane watch <pane_id> [--text] [--source visible|recent|recent-unwrapped] [--lines N] [--raw]";
...
fn print_pane_help() {
    eprintln!("momo pane commands:");
    eprintln!("  momo pane list [--workspace <workspace_id>]");
    eprintln!("  momo pane current [--pane ID|--current]");
    eprintln!("  momo pane get <pane_id>");
    eprintln!("  momo pane layout [--pane ID|--current]");
    eprintln!("  momo pane process-info [--pane ID|--current]");
    eprintln!("  momo pane neighbor --direction left|right|up|down [--pane ID|--current]");
    eprintln!("  momo pane edges [--pane ID|--current]");
    eprintln!("  momo pane focus --direction left|right|up|down [--pane ID|--current]");
    eprintln!(
        "  momo pane resize --direction left|right|up|down [--amount FLOAT] [--pane ID|--current]"
    );
    eprintln!("  momo pane zoom [<pane_id>|--pane ID|--current] [--toggle|--on|--off]");
    eprintln!("  momo pane rename <pane_id> <label>|--clear");
    eprintln!("  momo pane read <pane_id> [--source visible|recent|recent-unwrapped] [--lines N] [--format text|ansi] [--ansi]");
    eprintln!("  momo pane input [<pane_id>|--pane ID|--current] --right-click momo|pane");
    eprintln!(
        "  momo pane split [<pane_id>|--pane ID|--current] --direction right|down [--ratio FLOAT] [--cwd PATH] [--env KEY=VALUE] [--right-click momo|pane] [--focus] [--no-focus]"
    );
    eprintln!("  momo pane swap --direction left|right|up|down [--pane ID|--current]");
    eprintln!("  momo pane swap --source-pane ID --target-pane ID");
    eprintln!("  momo pane move <pane_id> --tab <tab_id> --split right|down [--target-pane ID] [--ratio FLOAT] [--focus|--no-focus]");
    eprintln!(
        "  momo pane move <pane_id> --new-tab [--workspace ID] [--label TEXT] [--focus|--no-focus]"
    );
    eprintln!("  momo pane move <pane_id> --new-workspace [--label TEXT] [--tab-label TEXT] [--focus|--no-focus]");
    eprintln!("  momo pane close <pane_id>");
    eprintln!("  momo pane send-text <pane_id> <text>");
    eprintln!("  momo pane send-keys <pane_id> <key> [key ...]");
    eprintln!("  momo pane wait-output <pane_id> (--match TEXT | --regex PATTERN) [--source visible|recent|recent-unwrapped] [--lines N] [--timeout MS] [--raw]");
    eprintln!("  momo pane watch <pane_id> [--text] [--source visible|recent|recent-unwrapped] [--lines N] [--raw]");
    eprintln!("  momo pane report-agent <pane_id> --source ID --agent LABEL --state idle|working|blocked|unknown [--message TEXT] [--seq N] [--agent-session-id ID] [--agent-session-path PATH]");
    eprintln!("  momo pane report-agent-session <pane_id> --source ID --agent LABEL [--seq N] [--agent-session-id ID] [--agent-session-path PATH]");
    eprintln!("  momo pane release-agent <pane_id> --source ID --agent LABEL [--seq N]");
    eprintln!("  momo pane report-metadata <pane_id> --source ID [--agent LABEL] [--applies-to-source ID] [--title TEXT|--clear-title] [--display-agent TEXT|--clear-display-agent] [--state-label STATUS=TEXT] [--clear-state-labels] [--token NAME=VALUE] [--clear-token NAME] [--seq N] [--ttl-ms N]");
    eprintln!("  momo pane run <pane_id> <command>");
}
...
mod tests {
    ...
    #[test]
    fn right_click_target_accepts_momo_and_the_older_herdr_name() {
        assert!(matches!(
            parse_right_click_target("momo"),
            Ok(PaneRightClickTarget::Herdr)
        ));
        assert!(matches!(
            parse_right_click_target("herdr"),
            Ok(PaneRightClickTarget::Herdr)
        ));
        assert!(parse_right_click_target("other").is_err());
    }
    ...
    fn parse_pane_swap_args_rejects_mixed_forms() {
        ...
        assert!(err.contains("usage: momo pane swap"));
    }
    ...
    fn parse_pane_move_args_rejects_target_pane_without_tab() {
        ...
        assert!(err.contains("usage: momo pane move"));
    }

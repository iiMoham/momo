...
fn plugin_link(args: &[String]) -> std::io::Result<i32> {
    let Some(path) = args.first() else {
        eprintln!("usage: momo plugin link <path> [--disabled]");
        return Ok(2);
    };
...
fn plugin_config_dir_command(args: &[String]) -> std::io::Result<i32> {
    let Some(plugin_id) = args.first() else {
        eprintln!("usage: momo plugin config-dir <plugin_id>");
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("usage: momo plugin config-dir <plugin_id>");
...
fn plugin_unlink(args: &[String]) -> std::io::Result<i32> {
    let Some(plugin_id) = args.first() else {
        eprintln!("usage: momo plugin unlink <plugin_id>");
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("usage: momo plugin unlink <plugin_id>");
...
fn plugin_install(args: &[String]) -> std::io::Result<i32> {
    let Some(source_arg) = args.first() else {
        eprintln!("usage: momo plugin install <owner>/<repo>[/subdir...] [--ref REF] [--yes]");
        return Ok(2);
    };
...
fn plugin_uninstall(args: &[String]) -> std::io::Result<i32> {
    let Some(target) = args.first() else {
        eprintln!("usage: momo plugin uninstall <plugin_id|owner/repo[/subdir...]>");
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("usage: momo plugin uninstall <plugin_id|owner/repo[/subdir...]>");
...
fn plugin_action_invoke(args: &[String]) -> std::io::Result<i32> {
    let Some(action_id) = args.first() else {
        eprintln!("usage: momo plugin action invoke <action_id> [--plugin ID]");
        return Ok(2);
    };
    let mut plugin_id = None;
    let mut index = 1;
...
fn plugin_pane_focus(args: &[String]) -> std::io::Result<i32> {
    let Some(pane_id) = args.first() else {
        eprintln!("usage: momo plugin pane focus <pane_id>");
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("usage: momo plugin pane focus <pane_id>");
...
fn plugin_pane_close(args: &[String]) -> std::io::Result<i32> {
    let Some(pane_id) = args.first() else {
        eprintln!("usage: momo plugin pane close <pane_id>");
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("usage: momo plugin pane close <pane_id>");
...
impl GithubPluginSource {
    fn parse(value: &str) -> Result<Self, String> {
        ...
        if parts.len() < 2 {
            return Err("usage: momo plugin install <owner>/<repo>[/subdir...]".into());
        }
        let owner = parts[0];
        let repo = parts[1];
...
fn verify_plugin_link_source_response(
        ...
        || plugin.source.managed_path != expected.managed_path
    {
        return Err(std::io::Error::other(
            "running MoMo server did not persist GitHub plugin source metadata",
        ));
    }
    Ok(())
}
...
fn plugin_checkout_lifecycle_error(operation: &str, path: &Path, err: io::Error) -> io::Error {
    if cfg!(windows) && err.kind() == io::ErrorKind::PermissionDenied {
        return io::Error::new(
            err.kind(),
            format!(
                "failed to {operation} managed plugin checkout at {}; close any MoMo plugin panes or plugin commands using that checkout, then retry: {err}",
                path.display()
            ),
        );
    }
...
fn print_plugin_help() {
    eprintln!("momo plugin commands:");
    eprintln!("  momo plugin install <owner>/<repo>[/subdir...] [--ref REF] [--yes]");
    eprintln!("  momo plugin uninstall <plugin_id|owner/repo[/subdir...]>");
    eprintln!("  momo plugin link <path> [--disabled]");
    eprintln!("  momo plugin list [--plugin ID] [--json]");
    eprintln!("  momo plugin config-dir <plugin_id>");
    eprintln!("  momo plugin unlink <plugin_id>");
    eprintln!("  momo plugin enable <plugin_id>");
    eprintln!("  momo plugin disable <plugin_id>");
    eprintln!("  momo plugin action <list|invoke>");
    eprintln!("  momo plugin log list [--plugin ID] [--limit N]");
    eprintln!("  momo plugin pane <open|focus|close>");
}

fn print_plugin_action_help() {
    eprintln!("momo plugin action commands:");
    eprintln!("  momo plugin action list [--plugin ID]");
    eprintln!("  momo plugin action invoke <action_id> [--plugin ID]");
}

fn print_plugin_pane_help() {
    eprintln!("momo plugin pane commands:");
    eprintln!("  momo plugin pane open --plugin ID --entrypoint ID [--placement overlay|popup|split|tab|zoomed] [--width SIZE] [--height SIZE] [--workspace ID] [--target-pane PANE] [--direction right|down] [--cwd PATH] [--env KEY=VALUE] [--focus|--no-focus]");
    eprintln!("  momo plugin pane focus <pane_id>");
    eprintln!("  momo plugin pane close <pane_id>");
}

...
fn server_stop(args: &[String]) -> std::io::Result<i32> {
    if !args.is_empty() {
        eprintln!("usage: momo server stop");
        return Ok(2);
    }
...
fn server_reload_config(args: &[String]) -> std::io::Result<i32> {
    if !args.is_empty() {
        eprintln!("usage: momo server reload-config");
        return Ok(2);
    }
...
fn server_agent_manifests(args: &[String]) -> std::io::Result<i32> {
    let json = match args {
        ...
        _ => {
            eprintln!("usage: momo server agent-manifests [--json]");
            return Ok(2);
        }
    };
...
fn server_reload_agent_manifests(args: &[String]) -> std::io::Result<i32> {
    if !args.is_empty() {
        eprintln!("usage: momo server reload-agent-manifests");
        return Ok(2);
    }
...
fn server_update_agent_manifests(args: &[String]) -> std::io::Result<i32> {
    let json = match args {
        ...
        [flag] if flag == "--json" => true,
        _ => {
            eprintln!("usage: momo server update-agent-manifests [--json]");
            return Ok(2);
        }
    };
...
fn server_live_handoff(args: &[String]) -> std::io::Result<i32> {
    let Some(params) = parse_live_handoff_params(args) else {
        eprintln!(
            "usage: momo server live-handoff [--import-exe <path>] [--expected-protocol <n>] [--expected-version <version>]"
        );
        return Ok(2);
    };
    ...
    eprintln!(
        ...
        crate::session::data_dir().join("momo-server.log").display()
    );
    Ok(0)
}
...
fn print_server_help() {
    eprintln!("momo server commands:");
    eprintln!("  momo server                run as headless server");
    eprintln!("  momo server stop           stop the running server via the API socket");
    eprintln!("  momo server live-handoff   hand off live panes to a new local server");
    eprintln!("  momo server reload-config  reload config.toml in the running server");
    eprintln!("  momo server agent-manifests [--json]  show agent detection manifest status");
    eprintln!(
        "  momo server update-agent-manifests [--json]  fetch and reload agent detection manifests"
    );
    eprintln!("  momo server reload-agent-manifests  reload agent detection manifests in the running server");
}

...
fn workspace_list(args: &[String]) -> std::io::Result<i32> {
    if !args.is_empty() {
        eprintln!("usage: momo workspace list");
        return Ok(2);
    }
...
fn workspace_get(args: &[String]) -> std::io::Result<i32> {
    let Some(raw_workspace_id) = args.first() else {
        eprintln!("usage: momo workspace get <workspace_id>");
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("usage: momo workspace get <workspace_id>");
...
fn workspace_focus(args: &[String]) -> std::io::Result<i32> {
    let Some(raw_workspace_id) = args.first() else {
        eprintln!("usage: momo workspace focus <workspace_id>");
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("usage: momo workspace focus <workspace_id>");
...
fn workspace_rename(args: &[String]) -> std::io::Result<i32> {
    if args.len() < 2 {
        eprintln!("usage: momo workspace rename <workspace_id> <label>");
        return Ok(2);
    }
...
fn workspace_report_metadata(args: &[String]) -> std::io::Result<i32> {
    let Some(raw_workspace_id) = args.first() else {
        eprintln!("usage: momo workspace report-metadata <workspace_id> --source ID [--token NAME=VALUE] [--clear-token NAME] [--seq N] [--ttl-ms N]");
        return Ok(2);
    };
...
fn workspace_close(args: &[String]) -> std::io::Result<i32> {
    let (raw_workspace_id, close_group) = match args {
        ...
        _ => {
            eprintln!("usage: momo workspace close <workspace_id> [--group]");
            return Ok(2);
        }
    };
...
fn print_workspace_help() {
    eprintln!("momo workspace commands:");
    eprintln!("  momo workspace list");
    eprintln!("  momo workspace create [--cwd PATH] [--label TEXT] [--env KEY=VALUE] [--focus] [--no-focus]");
    eprintln!("  momo workspace get <workspace_id>");
    eprintln!("  momo workspace focus <workspace_id>");
    eprintln!("  momo workspace rename <workspace_id> <label>");
    eprintln!("  momo workspace report-metadata <workspace_id> --source ID [--token NAME=VALUE] [--clear-token NAME] [--seq N] [--ttl-ms N]");
    eprintln!("  momo workspace close <workspace_id> [--group]");
}

...
fn agent_explain(args: &[String]) -> std::io::Result<i32> {
    ...
    while index < args.len() {
        match args[index].as_str() {
            ...
            "help" | "--help" | "-h" => {
                eprintln!("usage: momo agent explain <target> [--json|--verbose]");
                eprintln!("usage: momo agent explain --file PATH --agent LABEL [--json|--verbose]");
                return Ok(0);
            }
            ...
            value => {
                if target.is_some() {
                    eprintln!("usage: momo agent explain <target> [--json]");
                    return Ok(2);
                }
    ...
    let explain = if let Some(path) = file {
        if target.is_some() {
            eprintln!("usage: momo agent explain --file PATH --agent LABEL [--json]");
            return Ok(2);
        }
        let Some(agent_label) = agent else {
            eprintln!("momo agent explain --file requires --agent LABEL");
            return Ok(2);
        };
...
fn agent_start(args: &[String]) -> std::io::Result<i32> {
    let Some(name) = args.first() else {
        eprintln!("usage: momo agent start <name> --kind KIND --pane ID [--timeout MS] [-- <agent-args...>]");
        return Ok(2);
    };
    let separator = args
        .iter()
...
fn agent_list(args: &[String]) -> std::io::Result<i32> {
    if !args.is_empty() {
        eprintln!("usage: momo agent list");
        return Ok(2);
    }
...
fn agent_get(args: &[String]) -> std::io::Result<i32> {
    let Some(target) = args.first() else {
        eprintln!("usage: momo agent get <target>");
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("usage: momo agent get <target>");
...
fn agent_focus(args: &[String]) -> std::io::Result<i32> {
    let Some(target) = args.first() else {
        eprintln!("usage: momo agent focus <target>");
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("usage: momo agent focus <target>");
...
fn agent_attach(args: &[String]) -> std::io::Result<i32> {
    let (target, takeover) =
        match super::parse_attach_target(args, "usage: momo agent attach <target> [--takeover]") {
            Ok(parsed) => parsed,
...
fn agent_wait(args: &[String]) -> std::io::Result<i32> {
    let Some(target) = args.first() else {
        eprintln!("usage: momo agent wait <target> [--until STATUS]... [--timeout MS]");
        return Ok(2);
    };
...
fn agent_rename(args: &[String]) -> std::io::Result<i32> {
    let [target, value] = args else {
        eprintln!("usage: momo agent rename <target> <name>|--clear");
        return Ok(2);
    };
...
fn agent_prompt(args: &[String]) -> std::io::Result<i32> {
    let Some(target) = args.first() else {
        eprintln!(
            "usage: momo agent prompt <target> <text> [--wait] [--until STATUS]... [--timeout MS]"
        );
        return Ok(2);
    };
...
fn agent_send_keys(args: &[String]) -> std::io::Result<i32> {
    if args.len() < 2 {
        eprintln!("usage: momo agent send-keys <target> <key> [key ...]");
        return Ok(2);
    }
...
fn agent_read(args: &[String]) -> std::io::Result<i32> {
    let Some(target) = args.first() else {
        eprintln!("usage: momo agent read <target> [--source visible|recent|recent-unwrapped] [--lines N] [--format text|ansi] [--ansi]");
        return Ok(2);
    };
...
fn print_agent_help() {
    eprintln!("momo agent commands:");
    eprintln!("  momo agent list");
    eprintln!("  momo agent get <target>");
    eprintln!("  momo agent read <target> [--source visible|recent|recent-unwrapped|detection] [--lines N] [--format text|ansi] [--ansi]");
    eprintln!("  momo agent send-keys <target> <key> [key ...]");
    eprintln!("  momo agent prompt <target> <text> [--wait] [--until STATUS]... [--timeout MS]");
    eprintln!("  momo agent rename <target> <name>|--clear");
    eprintln!("  momo agent focus <target>");
    eprintln!("  momo agent wait <target> [--until STATUS]... [--timeout MS]");
    eprintln!("  momo agent attach <target> [--takeover]");
    eprintln!(
        "  momo agent start <name> --kind KIND --pane ID [--timeout MS] [-- <agent-args...>]"
    );
    eprintln!("  momo agent explain <target> [--json|--format text|json] [--verbose]");
    eprintln!(
        "  momo agent explain --file PATH --agent LABEL [--json|--format text|json] [--verbose]"
    );

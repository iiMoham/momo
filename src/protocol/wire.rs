...
pub fn check_client_version(client_version: u32) -> VersionCheck {
    ...
    if client_version == PROTOCOL_VERSION {
        ...
        VersionCheck::Incompatible(format!(
            "client version {client_version} is older than server version {PROTOCOL_VERSION}; please upgrade your momo client"
        ))
    } else {
        VersionCheck::Incompatible(format!(
            "client version {client_version} is newer than server version {PROTOCOL_VERSION}; please upgrade the momo server"
        ))
    }
}

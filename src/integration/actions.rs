...
pub(crate) fn uninstall_experimental_letta() -> io::Result<Vec<String>> {
    let result = uninstall_letta().map(|result| {
        ...
        if result.updated_settings {
            messages.push(format!(
                "removed momo letta hook entry from {}",
                result.settings_path.display()
            ));
        } else {
            messages.push(format!(
                "no momo letta hook entry found in {}",
                result.settings_path.display()
            ));
        }
        messages
    ...
    let mut messages = match target {
        ...
        crate::api::schema::IntegrationTarget::Droid => {
            ...
            if installed.updated_legacy_hooks {
                messages.push(format!(
                    "removed legacy momo droid hook entries from {}",
...
pub(crate) fn uninstall_target(
    ...
    let messages = match target {
        ...
        crate::api::schema::IntegrationTarget::Claude => {
            ...
            if result.updated_settings {
                messages.push(format!(
                    "removed momo claude hook entries from {}",
                    result.settings_path.display()
                ));
            } else {
                messages.push(format!(
                    "no momo claude hook entries found in {}",
                    result.settings_path.display()
                ));
            }
            messages
        ...
        crate::api::schema::IntegrationTarget::Codex => {
            ...
            if result.updated_hooks {
                messages.push(format!(
                    "removed momo codex hook entries from {}",
                    result.hooks_path.display()
                ));
            } else {
                messages.push(format!(
                    "no momo codex hook entries found in {}",
                    result.hooks_path.display()
                ));
            }
            messages.push(format!(
        ...
        crate::api::schema::IntegrationTarget::Copilot => {
            ...
            if result.updated_settings {
                messages.push(format!(
                    "removed momo copilot hook entries from {}",
                    result.settings_path.display()
                ));
            } else {
                messages.push(format!(
                    "no momo copilot hook entries found in {}",
                    result.settings_path.display()
                ));
            }
            messages
        ...
        crate::api::schema::IntegrationTarget::Devin => {
            ...
            if result.updated_settings {
                messages.push(format!(
                    "removed momo devin hook entries from {}",
                    result.settings_path.display()
                ));
            } else {
                messages.push(format!(
                    "no momo devin hook entries found in {}",
                    result.settings_path.display()
                ));
            }
            messages
        ...
        crate::api::schema::IntegrationTarget::Kimi => {
            ...
            if result.updated_config {
                messages.push(format!(
                    "removed momo kimi hook entries from {}",
                    result.config_path.display()
                ));
            } else {
                messages.push(format!(
                    "no momo kimi hook entries found in {}",
                    result.config_path.display()
                ));
            }
            messages
        ...
        crate::api::schema::IntegrationTarget::Droid => {
            ...
            if result.updated_hooks {
                ...
                messages.push(format!(
                    "no legacy momo droid hook entries found in {}",
                    result.hooks_path.display()
                ));
            }
            if result.updated_settings {
                messages.push(format!(
                    "removed momo droid hook entries from {}",
                    result.settings_path.display()
                ));
            } else {
                messages.push(format!(
                    "no momo droid hook entries found in {}",
                    result.settings_path.display()
                ));
            }
            messages
        ...
        crate::api::schema::IntegrationTarget::Opencode => {
            ...
            for path in result.updated_tui_configs {
                messages.push(format!(
                    "removed momo opencode plugin entry from {}",
                    path.display()
                ));
            }
            messages
        ...
        crate::api::schema::IntegrationTarget::Qodercli => {
            ...
            if result.updated_settings {
                messages.push(format!(
                    "removed momo qodercli hook entries from {}",
                    result.settings_path.display()
                ));
            } else {
                messages.push(format!(
                    "no momo qodercli hook entries found in {}",
                    result.settings_path.display()
                ));
            }
            messages
        ...
        crate::api::schema::IntegrationTarget::Qwen => {
            ...
            if result.updated_settings {
                messages.push(format!(
                    "removed momo qwen hook entries from {}",
                    result.settings_path.display()
                ));
            } else {
                messages.push(format!(
                    "no momo qwen hook entries found in {}",
                    result.settings_path.display()
                ));
            }
            messages
        ...
        crate::api::schema::IntegrationTarget::Cursor => {
            ...
            if result.updated_hooks {
                messages.push(format!(
                    "removed momo cursor hook entries from {}",
                    result.hooks_path.display()
                ));
            } else {
                messages.push(format!(
                    "no momo cursor hook entries found in {}",
                    result.hooks_path.display()
                ));
            }
            messages
        ...
        crate::api::schema::IntegrationTarget::Mastracode => {
            ...
            if result.updated_hooks {
                messages.push(format!(
                    "removed momo mastracode hook entries from {}",
                    result.hooks_path.display()
                ));
            } else {
                messages.push(format!(
                    "no momo mastracode hook entries found in {}",
                    result.hooks_path.display()
                ));
            }
            messages
        ...
        crate::api::schema::IntegrationTarget::AntigravityCli => {
            ...
            if result.updated_hooks {
                messages.push(format!(
                    "removed momo antigravity-cli hook entries from {}",
                    result.hooks_path.display()
                ));
            } else {
                messages.push(format!(
                    "no momo antigravity-cli hook entries found in {}",
                    result.hooks_path.display()
                ));
            }
            messages

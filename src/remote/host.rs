...
pub(crate) fn run_remote_client_bridge(args: &[String]) -> io::Result<()> {
    ...
    let stream = crate::ipc::connect_local_stream(&socket_path).map_err(|err| {
        io::Error::new(
            err.kind(),
            format!(
                "failed to connect to remote MoMo client socket {}: {err}",
                socket_path.display()
            ),
        )
    })?;
...
fn ensure_remote_server_running() -> io::Result<()> {
    ...
    if crate::server::autodetect::is_server_listening() {
        ...
        return Err(io::Error::other(
            "remote momo server needs one final update before this bridge can attach; rerun `momo --remote` from an interactive terminal to approve it",
        ));
    }

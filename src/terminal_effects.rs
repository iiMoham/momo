...
pub(crate) fn write_window_title<W: Write>(writer: &mut W, title: Option<&str>) -> io::Result<()> {
    let title = title.unwrap_or(crate::brand::CLI_NAME);
...
mod tests {
    ...
    #[test]
    fn window_title_strips_terminators_and_defaults_to_momo() {
        let mut output = Vec::new();
        write_window_title(&mut output, Some("momo\x1b api\u{7}\u{9c}")).unwrap();
        assert_eq!(output, b"\x1b]0;momo api\x07");

        output.clear();
        write_window_title(&mut output, None).unwrap();
        assert_eq!(output, b"\x1b]0;momo\x07");
    }
}

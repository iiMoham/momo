...
const TERMINAL_SESSION_OBSERVE_USAGE: &str =
    "usage: momo terminal session observe <target> [--cols N] [--rows N]";
const TERMINAL_SESSION_CONTROL_USAGE: &str =
    "usage: momo terminal session control <target> [--takeover] [--cols N] [--rows N]";
pub(crate) const AGENT_HELP_FOOTER: &str = concat!(
    "Are you an AI? Use these resources ONLY IF your task specifically asks you to:\n",
    "  Help a human understand or set up MoMo for the first time:\n",
    "    https://github.com/iiMoham/momo/blob/main/docs/agent-guide.md\n",
    "  Debug or investigate a problem with MoMo:\n",
    "    https://github.com/iiMoham/momo/blob/main/docs/troubleshooting.md\n",
    "  Control MoMo panes, agents, or workspaces:\n",
    "    SKIP if a MoMo skill is already in your context. Otherwise run: momo --skill",
);
...
fn channel_set(args: &[String]) -> std::io::Result<i32> {
    let Some(channel) = parse_channel_set_arg(args) else {
        eprintln!("usage: momo channel set <stable|preview>");
        return Ok(2);
    };
    ...
    println!(
        "MoMo update channel set to {channel} in {}.",
        path.display()
    );
    ...
    if let Err(err) = crate::update::self_update(crate::update::SelfUpdateOptions::default()) {
        ...
        eprintln!("Run `momo update` to retry.");
        return Ok(1);
    }
...
fn print_channel_help() {
    eprintln!("momo channel commands:");
    eprintln!("  momo channel show                  print the configured update channel");
    eprintln!("  momo channel set <stable|preview>  choose the update channel");
}
...
fn config_check(args: &[String]) -> std::io::Result<i32> {
    match args {
        ...
        [flag] if matches!(flag.as_str(), "help" | "--help" | "-h") => {
            eprintln!("usage: momo config check");
            return Ok(0);
        }
        _ => {
            eprintln!("usage: momo config check");
...
fn config_reset_keys(args: &[String]) -> std::io::Result<i32> {
    if !args.is_empty() {
        eprintln!("usage: momo config reset-keys");
        return Ok(2);
    }
    ...
    println!("Built-in v2 keybindings will apply after MoMo restarts or reloads config.");
    println!("If a MoMo server is running, run `momo server reload-config` to apply this now.");
    println!(
        "To restore: cp {} {}",
        backup_path.display(),
        path.display()
...
fn session_attach_help(args: &[String]) -> std::io::Result<i32> {
    if matches!(
        ...
        eprintln!("usage: momo session attach <name>");
        return Ok(0);
    }
    eprintln!("usage: momo session attach <name>");
    Ok(2)
...
fn session_list(args: &[String]) -> std::io::Result<i32> {
    let json = match parse_session_json_only(args, "usage: momo session list [--json]") {
        Ok(json) => json,
...
fn session_stop(args: &[String]) -> std::io::Result<i32> {
    let (name, json) =
        match parse_session_name_and_json(args, "usage: momo session stop <name> [--json]") {
            Ok(parsed) => parsed,
            Err(code) => return Ok(code),
        };
...
fn session_delete(args: &[String]) -> std::io::Result<i32> {
    let (name, json) =
        match parse_session_name_and_json(args, "usage: momo session delete <name> [--json]") {
            Ok(parsed) => parsed,
            Err(code) => return Ok(code),
        };
...
fn terminal_attach(args: &[String]) -> std::io::Result<i32> {
    let (terminal_id, takeover) = match parse_attach_target(
        args,
        "usage: momo terminal attach <terminal_id> [--takeover]",
    ) {
        Ok(parsed) => parsed,
        Err(code) => return Ok(code),
    };
...
fn terminal_title(args: &[String]) -> std::io::Result<i32> {
    match args.first().map(|arg| arg.as_str()) {
        Some("set") => {
            if args.len() != 2 {
                eprintln!("usage: momo terminal title set <title>");
                return Ok(2);
            }
        ...
        Some("clear") => {
            if args.len() != 1 {
                eprintln!("usage: momo terminal title clear");
                return Ok(2);
            }
            print_response(&send_request(&Request {
        ...
        Some("help" | "--help" | "-h") => {
            eprintln!("usage: momo terminal title set <title>");
            eprintln!("       momo terminal title clear");
            Ok(0)
        }
        _ => {
            eprintln!("usage: momo terminal title set <title>");
...
fn print_config_help() {
    eprintln!("momo config commands:");
    eprintln!("  momo config check  validate config.toml and print diagnostics");
    eprintln!("  momo config reset-keys  back up config.toml and remove custom keybindings");
}

fn print_terminal_help() {
    eprintln!("momo terminal commands:");
    eprintln!("  momo terminal attach <terminal_id> [--takeover]");
    eprintln!("  momo terminal session control <target> [--takeover] [--cols N] [--rows N]");
    eprintln!("  momo terminal session observe <target> [--cols N] [--rows N]");
    eprintln!("  momo terminal title set <title>");
    eprintln!("  momo terminal title clear");
    eprintln!("  detach from direct attach with ctrl+b q; send literal ctrl+b with ctrl+b ctrl+b");
}

fn print_session_help() {
    eprintln!("momo session commands:");
    eprintln!("  momo session list [--json]");
    eprintln!("  momo session attach <name>");
    eprintln!("  momo session stop <name> [--json]");
    eprintln!("  momo session delete <name> [--json]");

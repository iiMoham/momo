...
/// Plugin manifest file names, preferred first. `herdr-plugin.toml` is the
/// older name and keeps existing plugins installable.
pub(crate) const PLUGIN_MANIFEST_NAMES: [&str; 2] = ["momo-plugin.toml", "herdr-plugin.toml"];

/// The manifest inside a plugin folder: `momo-plugin.toml` when present,
/// otherwise the older `herdr-plugin.toml`.
pub(crate) fn plugin_manifest_in(dir: &std::path::Path) -> std::path::PathBuf {
    PLUGIN_MANIFEST_NAMES
        .iter()
        .map(|name| dir.join(name))
        .find(|candidate| candidate.is_file())
        .unwrap_or_else(|| dir.join(PLUGIN_MANIFEST_NAMES[0]))
}
...
fn validate_min_herdr_version(value: Option<&str>) -> Result<String, (&'static str, String)> {
    ...
    if required > current {
        return Err((
            ...
            format!("plugin requires MoMo {required} or newer; current MoMo is {current}"),
        ));
    }
...
mod manifest_name_tests {
    ...
    #[test]
    fn plugin_folders_prefer_momo_manifest_and_accept_the_older_name() {
        let dir = std::env::temp_dir().join(format!("momo-manifest-name-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
        assert_eq!(plugin_manifest_in(&dir), dir.join("momo-plugin.toml"));

        std::fs::write(dir.join("herdr-plugin.toml"), "id = 'old.name'\n").unwrap();
        assert_eq!(plugin_manifest_in(&dir), dir.join("herdr-plugin.toml"));

        std::fs::write(dir.join("momo-plugin.toml"), "id = 'new.name'\n").unwrap();
        assert_eq!(plugin_manifest_in(&dir), dir.join("momo-plugin.toml"));

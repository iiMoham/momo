...
pub enum ToastDelivery {
    #[default]
    Off,
    /// In-app toasts. Written as `momo`; `herdr` is the older spelling.
    #[serde(rename = "momo", alias = "herdr")]
    Herdr,
    Terminal,
    System,
}
...
pub struct ToastConfig {
    ...
    /// `[ui.toast.momo]`; `[ui.toast.herdr]` is the older spelling.
...
pub struct KeysConfig {
    ...
    /// Toggle broadcasting typing to every pane in the focused tab. Default: unset
    pub toggle_input_sync: BindingConfig,
...
pub(crate) struct KeysConfigOverlay {
    ...
    toggle_input_sync: Option<BindingConfig>,
    #[serde(skip_serializing_if = "Option::is_none")]
...
impl KeysConfig {
    ...
    pub(crate) fn local_profile(&self, keybinds: &Keybinds) -> KeysConfigOverlay {
        ...
        copy_effective_action_field!(toggle_input_sync, keybinds.toggle_input_sync);
...
impl Default for KeysConfig {
    fn default() -> Self {
        Self {
            ...
            toggle_input_sync: BindingConfig::empty(),
...
"#,
    ...
    fn pane_appearance_defaults_and_parse() {
        ...
        assert_eq!(default_config.ui.tab_bar_padding, 0);
...
"#;
        ...
        assert_eq!(config.ui.tab_bar_padding, 1);
    ...
    fn worktrees_directory_defaults_and_parses() {
        let default_config = Config::default();
        assert_eq!(default_config.worktrees.directory, "~/.momo/worktrees");
...
"#;
    ...
    #[test]
    fn toast_config_accepts_momo_names_and_the_older_herdr_names() {
        for (delivery, table) in [("momo", "momo"), ("herdr", "herdr")] {
            let toml = format!(
                "[ui.toast]\ndelivery = \"{delivery}\"\n[ui.toast.{table}]\nposition = \"top-left\"\n"
            );
            let config: Config = toml::from_str(&toml).unwrap();
            assert_eq!(config.ui.toast.delivery, ToastDelivery::Herdr, "{toml}");
            assert_eq!(
        ...
        assert_eq!(
            serde_json::to_string(&ToastDelivery::Herdr).unwrap(),
            "\"momo\""
        );
    }

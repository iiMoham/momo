...
mod tests {
    ...
    fn release_notes_inline_code_spans_are_styled_without_backticks() {
        ...
        let lines = release_notes_lines("- `momo pane run ...` now works", &palette);

        assert_eq!(lines.len(), 1);
        assert_eq!(line_text(&lines[0].1), " • momo pane run ... now works");
        assert_eq!(lines[0].1.spans[1].content.as_ref(), "momo pane run ...");
    ...
    fn release_notes_preview_lines_show_update_steps() {
        let palette = Palette::catppuccin();
        let lines = release_notes_preview_line_entries("momo update", &palette)
            .into_iter()
            .map(|(_, line)| line)
            .collect::<Vec<_>>();
        ...
        assert_eq!(lines[1].spans[2].content.as_ref(), "momo update");
    ...
    fn release_notes_preview_display_is_part_of_the_scrollable_notes_body() {
        ...
        let lines = release_notes_display_lines(&notes, "momo update", &palette);

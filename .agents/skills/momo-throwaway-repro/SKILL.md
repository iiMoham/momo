---
name: momo-throwaway-repro
description: Create and control a disposable named MoMo session from inside an existing MoMo session. Use for isolated MoMo runtime, pane, terminal, process, API, persistence, or agent reproductions that should be driven through the CLI/API without touching the default session.
---

# MoMo throwaway reproduction

Use a disposable named MoMo session when a reproduction needs a real MoMo server, panes, PTYs, agents, or socket API without risking the user's main session.

The temporary TUI only keeps the disposable session attached and supplies terminal geometry. Drive the reproduction from the parent session through MoMo's CLI/API. Do not manually operate the nested TUI unless the bug specifically requires client input.

## Non-negotiable safety

- Never run the reproduction in the default session.
- Never stop, restart, delete, or kill the main MoMo server.
...
Confirm the caller is inside MoMo and inspect the relevant help before doing anything:
...
Inspect nested command help before using unfamiliar or potentially mutating commands. Do not run bare `momo` for discovery because it launches or attaches the TUI.

Record which MoMo binary and version the reproduction tests. If testing a checkout build, follow the repository's instructions for running that build instead of silently substituting the installed binary.

## Create the outer pane

Create a sibling shell pane in the current tab without moving focus. Use an available MoMo layout tool when the harness provides one. Otherwise use the installed pane split command after checking its help.
...
Validate the test config with `momo config check` using the same config override
before launch. After launch, read the outer pane to catch startup errors such as
`nested momo is disabled by default`; do not assume the launch succeeded merely
...
Named sessions isolate runtime state, sockets, panes, and persistence. They still share global MoMo configuration and agent manifest overrides by default. Check configuration provenance when it could affect the reproduction. Do not modify shared configuration merely to make the test pass.
...
Use agent commands only after MoMo recognizes a coding agent:
...
Before launching an agent, inspect its installed `--version` and `--help`. Pass native agent arguments after MoMo's argument separator.
...
Read the named session directory and socket from `momo session list` instead of assuming their paths. Keep large evidence under `/var/tmp` unless the user asks to preserve it elsewhere.

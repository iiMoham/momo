...
fn main() {
    ...
    println!("cargo:rerun-if-env-changed=MOMO_BUILD_CHANNEL");
    println!("cargo:rerun-if-env-changed=MOMO_BUILD_ID");
    println!("cargo:rerun-if-env-changed=MOMO_BUILD_COMMIT");
    ...
    assert!(
        ...
        "zig build for vendored libghostty-vt failed: {status}. \
         Building MoMo requires Zig 0.16.0; check `zig version` \

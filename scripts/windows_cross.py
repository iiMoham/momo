...
SDK_ROOT = Path.home() / ".local/share/momo/windows-cross"
...
def setup(accept_license: bool) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="momo-windows-sdk-", dir=temp_parent) as cache:
...
def lint() -> None:
    ...
    subprocess.run(
        ["cargo", "clippy", "--bin", "momo", "--locked", "--target", TARGET, "--", "-D", "warnings"],
        env=env,
        check=True,
    )

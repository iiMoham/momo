...
Seeds a running momo server with navigator demo workspaces, tabs, panes,
and fake agent states for recording the session navigator.

Environment:
  HERDR_NAV_SOCKET_PATH  API socket to target. Defaults to $HOME/.config/momo-dev/herdr.sock.

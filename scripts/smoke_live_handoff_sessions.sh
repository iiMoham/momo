...
HERDR_BIN="${HERDR_BIN:-$ROOT/target/debug/momo}"
...
session_dir() {
  ...
  if [[ "$session" == "default" ]]; then
    printf '%s/momo-dev' "$CONFIG_HOME"
  else
    printf '%s/momo-dev/sessions/%s' "$CONFIG_HOME" "$session"
  fi
}
...
assert_smoke_socket() {
  local socket="$1"
  case "$socket" in
    "$CONFIG_HOME"/momo-dev/herdr.sock | "$CONFIG_HOME"/momo-dev/sessions/*/herdr.sock)
      ;;
    *)
...
mkdir -p "$CONFIG_HOME/momo-dev" "$RUNTIME_DIR" "$STATE_DIR"
printf 'onboarding = false\n' > "$CONFIG_HOME/momo-dev/config.toml"

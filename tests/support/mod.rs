...
fn start_global_watchdog() {
    thread::spawn(|| loop {
        ...
        if let Err(err) = cleanup_servers_with_missing_runtime_dir() {
            eprintln!("momo test cleanup watchdog error: {err}");
        }
    });
}
...
fn is_test_herdr_binary(path: &Path) -> bool {
    ...
    TEST_BINARY
        .get_or_init(|| fs::canonicalize(env!("CARGO_BIN_EXE_momo")).ok())
        .as_deref()
...
mod tests {
    ...
    fn test_binary_matcher_accepts_cargo_test_binary() {
        let binary = std::fs::canonicalize(env!("CARGO_BIN_EXE_momo"))
    ...
    fn test_binary_matcher_rejects_other_binaries() {
        ...
        let sibling_build = Path::new(env!("CARGO_BIN_EXE_momo"))
            .parent()
            .unwrap()

...
fn config_check_reports_unknown_theme_names() {
    ...
    assert!(stdout.contains("config: issues found"), "{stdout}");
    assert!(
        stdout.contains("unknown theme name theme.name = \"catppucin\"; using \"midnight-neon\""),
        "{stdout}"
    );
    assert!(
        stdout.contains("valid themes: midnight-neon, neon-day, catppuccin"),
        "{stdout}"
    );
...
fn worktree_cli_rejects_local_argument_errors_before_socket_use() {
    ...
    for args in cases {
        ...
        assert_eq!(
            output.status.code(),
            Some(2),
            "momo {} should fail as local parse error; stdout={} stderr={}",
            args.join(" "),

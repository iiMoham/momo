...
fn dead_server_cli_reports_one_session_aware_json_line() {
    ...
    assert_server_not_running(missing, &named_socket, "momo session attach foo");

    let stale_socket = runtime_dir.join("stale.sock");
    drop(UnixListener::bind(&stale_socket).unwrap());
    let stale = Command::new(env!("CARGO_BIN_EXE_momo"))
        ...
        .output()
        .unwrap();
    assert_server_not_running(stale, &stale_socket, "momo");
...
fn integration_commands_run_locally_when_server_is_missing() {
    ...
    let workspace_list = Command::new(env!("CARGO_BIN_EXE_momo"))
        .args(["workspace", "list"])
    ...
    let integration_install = Command::new(env!("CARGO_BIN_EXE_momo"))
    ...
    let integration_status = Command::new(env!("CARGO_BIN_EXE_momo"))
    ...
    let integration_uninstall = Command::new(env!("CARGO_BIN_EXE_momo"))
...
fn integration_status_outdated_only_prints_action_for_legacy_install() {
    ...
    assert!(stderr.contains("installed momo integrations need updating"));
    assert!(stderr.contains("momo integration install pi"));
...
fn unloaded_session_survives_autosave_and_shutdown_when_recovery_is_blocked() {
    ...
    assert!(wait_until(
        ...
        || {
            fs::read_to_string(data_dir.join("momo-server.log"))

...
fn help_commands_exit_successfully() {
    ...
    for args in help_cases {
        ...
        assert!(
            ...
            "momo {} failed: status={:?} stdout={} stderr={}",
            args.join(" "),
            output.status.code(),
...
fn root_and_command_group_help_point_agents_to_plain_text_docs() {
    for args in [&["--help"][..], &["agent", "--help"][..]] {
        let output = Command::new(env!("CARGO_BIN_EXE_momo"))
            ...
            .output()
            .unwrap();
        assert!(output.status.success(), "momo {} failed", args.join(" "));
        ...
        for expected in [
            ...
            assert!(
                ...
                "momo {} help did not contain {expected:?}: {stdout}",
                args.join(" ")
            );
        }
    }
...
fn removed_wait_and_agent_send_commands_are_rejected() {
    let wait = Command::new(env!("CARGO_BIN_EXE_momo"))
    ...
    let help = Command::new(env!("CARGO_BIN_EXE_momo"))
        .arg("--help")
        .output()
        .unwrap();
    assert!(!String::from_utf8_lossy(&help.stdout).contains("momo wait <subcommand>"));

    let send = Command::new(env!("CARGO_BIN_EXE_momo"))
        .args(["agent", "send", "reviewer", "hello"])
        .output()
        .unwrap();
    assert_eq!(send.status.code(), Some(2));
    let stderr = String::from_utf8_lossy(&send.stderr);
    assert!(stderr.contains("momo agent send-keys"));
    assert!(!stderr.contains("momo agent send <"));
}
...
fn completion_command_prints_zsh_script_without_session_startup() {
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(stdout.contains("#compdef momo"), "stdout: {stdout}");
    assert!(
        !stdout.contains("--cwd=[]"),
...
fn root_help_advertises_api_schema_command_group() {
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(
        stdout.contains("momo api <subcommand>"),
...
fn api_schema_default_output_is_a_short_summary() {
    ...
    assert!(output.status.success());
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(stdout.contains("MoMo API schema"), "stdout: {stdout}");
    assert!(
        stdout.contains("Use `momo api schema --json`"),
        "stdout: {stdout}"
    );
    assert!(
        stdout.len() < 400,
